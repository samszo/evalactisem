...
class Xul{
  ...
  private $site;
   ...
   function Get_Tree_NoTrad_Uti($idUti){
	...
	//r�cup�re les tags non traduits
...
function GetTreeItemTradUti($idUti,$type){
	...
	if($idUti==$this->site->infos["UTI_TRAD_AUTO"]){
		//des traduction automatiques partag�es par l'utilisateur 
	...
	$i=0;
   	while($r = mysql_fetch_assoc($rs)){
   		...
   		//pour g�rer le changement de branche parente
       	if($r["uti_id"]!=$oIdUti){
       		$oFluxCode = -1;
       	}
		...
		//v�rifie si on change d'utilisateur
       	if($r["uti_id"]!=$oIdUti){
       		if($i>0){
	       		//on ferme le pr�c�dent utilisateur
				$ihm .= $ihmTags;
	       		$ihm .= '</treechildren>'.EOL;
				$ihm .= '</treeitem>'.EOL;
       		}
